...
We will use the domain `millironsgroup.com` which will point to our local VM. If you want to use a different domain, make sure to change the domain in all files (Apache conf files, JS files, etc.)


Create dir and parents if they do not exist:

```
sudo mkdir -p /etc/ssl/localcerts/millironsgroup.com/
```
...
-keyout /etc/ssl/localcerts/millironsgroup.com/privkey.pem -out /etc/ssl/localcerts/millironsgroup.com/fullchain.pem \
...
```
sudo chmod 600 /etc/ssl/localcerts/millironsgroup.com/privkey.pem
```
...
We now need to add new entries to our hosts file, to point the domain used in this demo `millironsgroup.com` and all used subdomains to our VM on which Apache and Evilginx are running.
...
[IP] login.millironsgroup.com
[IP] account.millironsgroup.com
[IP] sso.millironsgroup.com
[IP] www.millironsgroup.com
[IP] portal.millironsgroup.com
[IP] millironsgroup.com
# End of section
```
...
Since we are using self-signed SSL certificates, our browser will warn us every time we try to visit `millironsgroup.com` so we need to make our host machine trust the certificate authority that signed the SSL certs.

For this step, it's easier to follow the video instructions, but here is the gist anyway.



Open [https://millironsgroup.com/](https://millironsgroup.com/) in your Chrome browser.
...
You should be able to visit `https://millironsgroup.com` now and see the homepage without any SSL warnings.
...
```
config domain millironsgroup.com
```
...
```
phishlets hostname O365 millironsgroup.com
```
